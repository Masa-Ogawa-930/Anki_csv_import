    ...
    fn gather_note_fields(&self, record: &csv::StringRecord) -> Vec<Option<String>> {
        ...
        self.field_source_columns
            ...
            .map(|opt| {
                ...
                .map(|s| stringify(strip_outer_quotes_and_trim(s))) // 前後の空白と引用符を削除して stringify
            })
            .collect()
}
...
fn strip_outer_quotes_and_trim(s: &str) -> &str {
    let s = s.trim(); // eliminating blanks and quotation marks
    if s.starts_with('"') && s.ends_with('"'){
        &s[1..s.len()-1].trim() // 先頭と末尾の引用符を削除して再度 trim
    }else if s.starts_with('"'){
        &s[1..s.len()].trim() // 先頭の引用符を削除して再度 trim
    }else if s.ends_with('"'){
        &s[0..s.len()-1].trim()// 末尾の引用符を削除して再度 trim
    } 
     else {
        s
    }
